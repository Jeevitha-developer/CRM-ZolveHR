...
export function RegisterForm({ onRegisterSuccess, onSwitchToLogin, loading = false, error }: RegisterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message.includes('Failed to fetch') || error.message.includes('Network Error') || error.code === 'ECONNREFUSED') {
        const backendError = 'Backend server is not running. Please start the backend server on port 5000 and ensure the database is initialized.';
      ...
      if (error.response && error.response.data && error.response.data.details) {
        const validationErrors = error.response.data.details.map((detail: any) => detail.message).join(', ');
